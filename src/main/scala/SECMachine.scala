...
class SECMachine (emitter : Emitter) extends PrettyPrinter {
    ...
    def exec (instr : Instr, state : State) : Status = {
        ...
        // print list like a Haskell list rather than a Scala list
        def printList(s:List[Int]):String = s match
        {
        case List() => "[]"
        case List(a) => "[" + a + "]"
        case h :: t => "[" + h + (t map (", " + _)).reduce(_ + _) + "]"
        }
                    ...
                    case value :: opndstail =>
                        value match {
                            ...
                            case MBool (value) =>
                                emitter.emitln (value)
                            case MList (value) =>
                                emitter.emitln (printList(value))
